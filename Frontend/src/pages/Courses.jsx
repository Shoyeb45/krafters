    ...
    <div className='w-64 bg-white border-r border-gray-200 h-screen flex flex-col flex-shrink-0'>
      <div className='p-4 border-b border-gray-200'>
        <div className='flex items-center space-x-3'>
          <img src='' alt='' />
          <span className='font-semibold text-lg text-gray-800'>Physics Wallah</span>
        </div>
      </div>
      ...
      <nav className='flex-1 py-4'>
        {menuItems.map((item, index) => {
          ...
          return (
            <div
              key={index}
              className={`flex items-center px-4 py-3 mx-2 rounded-lg cursor-pointer transition-colors ${item.active
...
const MainContent = ({ onCourseSelect }) => {
  ...
  useEffect(async () => {
    try {
      const res = await axios.get('https://amanrajgupta.me/api/course/get-all');
        ...
        <div className='flex items-center justify-between'>
          ...
          <div className='flex items-center space-x-4'>
            <div className='flex items-center space-x-2 text-gray-600 px-3 py-1 bg-slate-200 rounded-full'>
              <img src='' alt='batch' />
              <span className='text-[1.5em] font-extrabold text-black'>420</span>
            </div>
            <div className='flex items-center space-x-2'>
              <span className='text-sm text-black font-semibold'>Hi, Sazid</span>
          ...
          <div className='flex justify-around items-center gap-6 mt-2 mb-4'>
            {filters.map((filter, index) => (
              <button
                ...
                className={`p-2 cursor-pointer transition-all duration-200 border-b-2 hover:bg-gray-100 rounded-t-md ${activeFilter === index
            ...
            {batches ? batches.map((batch) => (
              ...
              >
                ...
                <div className='relative w-[270px] h-[160px] bg-slate-400 my-2 mb-7 '>
                  <img src={batch.thumbnail.secure_url} alt='batch' className='absolute w-full h-full top-0 left-0' />
                </div>
                <div>
                  {batch.category}
                ...
                <div className='flex justify-center items-center gap-7 mt-5 '>
                  <div className='flex justify-center items-center gap-1'>
                    <p className='text-lg font-semibold'>&#8377;{batch.price-100}</p>
                    <p className='line-through text-gray-500'>{batch.price}</p>
                  </div>
                ...
                <div className='p-4 flex justify-center items-center gap-7'>
                </div>
              </div>
            ))
              : (
